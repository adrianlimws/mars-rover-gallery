import { c as create_ssr_component, f as compute_rest_props, h as spread, i as escape_object, o as onDestroy, b as add_attribute, j as createEventDispatcher, a as subscribe, e as escape, v as validate_component, k as compute_slots } from "./ssr.js";
...
const LightboxFooter = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  })}><h2><!-- HTML_TAG_START -->${title}<!-- HTML_TAG_END --></h2> <h5><!-- HTML_TAG_START -->${description}<!-- HTML_TAG_END --></h5> ${gallery !== null ? `<p>${escape(localizedGalleryCounter)}</p>` : ``} </div>`;
});
const css$1 = {
...
const ModalCover = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div${spread([{ class: "svelte-lightbox-overlay" }, escape_object($$restProps)], { classes: "svelte-7wg54p" })}>${slots.default ? slots.default({}) : ``} </div>`;
});
const css = {
